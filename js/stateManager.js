...
export class StateManager {
    ...
    // 🔓 Unlock all modules
    unlockAllModules() {
        if (this.userProgress.techniques) {
            for (const technique of Object.values(this.userProgress.techniques)) {
                technique.mastered = true; // mark as mastered
                for (const sub of Object.keys(technique.subSkills)) {
                    technique.subSkills[sub] = ["step1","step2","step3"]; // fake all complete
                }
            }
            this.saveProgress();
        }
